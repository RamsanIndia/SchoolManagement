...
{
    public class ActivateClassCommandHandler : IRequestHandler<ActivateClassCommand, Result>
    {
        private readonly IClassRepository _classRepository;
        private readonly IUnitOfWork _unitOfWork;
        private readonly ILogger<ActivateClassCommandHandler> _logger;
        ...
        public ActivateClassCommandHandler(
            ...
            ILogger<ActivateClassCommandHandler> logger,
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                _logger.LogInformation(
                    "Starting class activation. CorrelationId: {CorrelationId}, ClassId: {ClassId}",
                ...
                // ✅ Pass correlationId
                classEntity.Activate(userId);

                await _unitOfWork.SaveChangesAsync(cancellationToken);

                _logger.LogInformation(
                    "Class activated successfully. CorrelationId: {CorrelationId}, ClassId: {ClassId}",
            ...
            {
                _logger.LogError(ex,
                    "Domain validation failed during class activation. CorrelationId: {CorrelationId}, ClassId: {ClassId}",
                    correlationId, request.ClassId);
            ...
            {
                _logger.LogError(ex,
                    "Unexpected error during class activation. CorrelationId: {CorrelationId}, ClassId: {ClassId}",
                    correlationId, request.ClassId);
                return Result.Failure("Failed to activate class. Please try again later.");
            }
        }
    }
}
